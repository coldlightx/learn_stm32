...
void MPU6050_read_data_uint16(uint8_t reg_addr_start, uint16_t *arr, uint8_t length)
{
    MyI2C_read_data_uint16(MPU6050_ADDR, reg_addr_start, arr, length);
}
...
{
    MPU6050_read_data_uint16(MPU6050_DATA_REG_START, (uint16_t *)data, 7);
}

uint8_t MPU6050_get_id()
{
    return MPU6050_read_byte(WHO_AM_I);
}